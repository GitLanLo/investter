...
Goal:
make the sequence dataset large and clean enough for a first neural run.
...
Goal:
create leakage-safe neural input windows from the Sprint 7 selected tabular feature dataset.
...
Goal:
train a small sequence model that is simple enough to debug and strong enough to compare fairly.
...
Goal:
decide whether the neural model improves the Sprint 7 baseline or clearly fails with evidence.
...
Goal:
make the neural artifact consumable by runtime services without guessing shapes, feature order or normalization.
...
Goal:
let the operator and API see the Sprint 8 neural artifact status without pretending that unsupported runtime inference works.
...
Goal:
show Sprint 8 neural evidence and runtime status in the operator GUI.
...
Goal:
make Sprint 8 repeatable without requiring a full expensive training run in every check.
...
- Sequence dataset manifests exist for selected Sprint 7 default `1h/h24` and at least one window size.
- At least one neural sequence model is trained and evaluated.
- Neural results are compared against the Sprint 7 `hgb_multiclass` baseline.
- The conclusion is documented as candidate, rejected, or blocked by data volume.
- A deployable model artifact directory and manifest exist.
- Backend can read and report the neural manifest and runtime status.
- Frontend shows Sprint 8 neural evidence without breaking when artifacts are absent.
